...
import { FAQ_CONTAINER_CONFIGURATION_LOADED } from '../store/actions';

export class FAQSocketListener extends SocketListener {
    ...
    private initConfigurationSocketListener(socket: SocketIO.Server): void {
        ...
        socket.on(ConfigurationEvent.COMPONENT_CONFIGURATION_LOADED,
            (result: LoadConfigurationResult<ContainerConfiguration>) => {
                this.store.dispatch(FAQ_CONTAINER_CONFIGURATION_LOADED(result.configuration));
            });
