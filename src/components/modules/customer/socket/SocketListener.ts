...
import {
    CUSTOMER_CONTAINER_CONFIGURATION_LOADED
} from '../store/actions';

export class ServicesSocketListener extends SocketListener {
    ...
    private initConfigurationSocketListener(socket: SocketIO.Server): void {
        ...
        socket.on(ConfigurationEvent.COMPONENT_CONFIGURATION_LOADED,
            (result: LoadConfigurationResult<ContainerConfiguration>) => {
                this.store.dispatch(CUSTOMER_CONTAINER_CONFIGURATION_LOADED(result.configuration));
            });
