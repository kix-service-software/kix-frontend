...
class StandardTableComponent<T extends KIXObject<T>> {
    ...
    public getToggleActions(kixObject: KIXObject): IAction[] {
        ...
        if (this.state.standardTable.isToggleEnabled()) {
            actions = ActionFactory.getInstance().generateActions(
                this.state.standardTable.tableConfiguration.toggleOptions.actions, [kixObject]
            );
        }
        return actions;
    }
    ...
    public getValueClasses(value: TableValue, column: TableColumn): string[] {
    ...
    public getRoutingConfiguration(column: TableColumn): RoutingConfiguration {
        let config;
