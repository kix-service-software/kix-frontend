...
import {
    KIXObjectPropertyFilter, TableFilterCriteria, KIXObjectType, TicketTypeProperty
} from '../../../../core/model';
...
class Component extends AbstractMarkoComponent<ComponentState> {
    ...
    public async onMount(): Promise<void> {
        this.state.predefinedTableFilter = [
            new KIXObjectPropertyFilter('Gültig', [
                new TableFilterCriteria(TicketTypeProperty.VALID_ID, SearchOperator.EQUALS, 1, false)
            ]),
            new KIXObjectPropertyFilter('Ungültig', [
                new TableFilterCriteria(TicketTypeProperty.VALID_ID, SearchOperator.EQUALS, 2, false)
            ]),
    ...
    private async prepareTitle(): Promise<void> {
        ...
        const typeName = await labelProvider.getObjectName(true);
