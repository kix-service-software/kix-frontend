...
export class MailFilterTableDeleteAction extends AbstractAction<Table> {
    ...
    public async run(): Promise<void> {
        if (this.canRun()) {
            ...
            const content = new ComponentContent(
                'confirm-overlay', new ConfirmOverlayContent(question, this.deleteFilter.bind(this))
            );
    ...
    public async deleteFilter(): Promise<void> {
        const selectedRows = this.data.getSelectedRows();
        if (selectedRows && !!selectedRows.length) {
            EventService.getInstance().publish(ApplicationEvent.APP_LOADING, {
                loading: true, hint: 'Translatable#Remove Filter'
            });
            const failIds = await KIXObjectService.deleteObject(
                KIXObjectType.MAIL_FILTER, selectedRows.map((sR) => sR.getRowObject().getObject().ObjectId)
            );
            ...
            if (!failIds || failIds.length === 0) {
                const content = new ComponentContent(
                    'toast',
                    new ToastContent('kix-icon-check', 'Translatable#Filter successfully removed.')
                );
