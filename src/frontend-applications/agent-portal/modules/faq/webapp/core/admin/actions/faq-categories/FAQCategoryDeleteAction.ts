...
export class FAQCategoryDeleteAction extends AbstractAction<Table> {
    ...
    public async run(): Promise<void> {
        if (this.canRun()) {
            ...
            const question = await TranslationService.translate(
                'Translatable#The following {0} entries and existing sub categories will be deleted. Are you sure?',
                [selectedRows.length]
            );
            const content = new ComponentContent(
                'confirm-overlay', new ConfirmOverlayContent(question, this.deleteCategories.bind(this))
            );
            ...
            OverlayService.getInstance().openOverlay(
                ...
                null,
                content,
                'Translatable#Remove FAQ Categories',
                null,
                false
            );
        }
    ...
    public async deleteCategories(): Promise<void> {
        ...
        if (selectedRows && !!selectedRows.length) {
            EventService.getInstance().publish(ApplicationEvent.APP_LOADING, {
                loading: true, hint: 'Translatable#Remove FAQ Categories'
            });
            const failIds = await KIXObjectService.deleteObject(
                KIXObjectType.FAQ_CATEGORY, selectedRows.map((sR) => sR.getRowObject().getObject().ObjectId)
            );
            ...
            if (!failIds || !failIds.length) {
                const content = new ComponentContent(
                    'toast',
                    new ToastContent('kix-icon-check', 'Translatable#FAQ categories successfully removed.')
                );
