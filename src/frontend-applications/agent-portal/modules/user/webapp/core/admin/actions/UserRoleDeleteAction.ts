...
export class UserRoleDeleteAction extends AbstractAction<Table> {
    ...
    public async run(): Promise<void> {
        if (this.canRun()) {
            ...
            const content = new ComponentContent(
                'confirm-overlay', new ConfirmOverlayContent(question, this.deleteRoles.bind(this))
            );
    ...
    public async deleteRoles(): Promise<void> {
        ...
        if (selectedRows && !!selectedRows.length) {
            EventService.getInstance().publish(ApplicationEvent.APP_LOADING, {
                loading: true, hint: 'Translatable#Remove Roles'
            });
            const failIds = await KIXObjectService.deleteObject(
                KIXObjectType.ROLE, selectedRows.map((sR) => sR.getRowObject().getObject().ObjectId)
            );
            ...
            if (!failIds || failIds.length === 0) {
                const content = new ComponentContent(
                    'toast',
                    new ToastContent('kix-icon-check', 'Translatable#Roles successfully removed.')
                );
