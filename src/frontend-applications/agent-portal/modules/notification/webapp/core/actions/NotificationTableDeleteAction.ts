...
export class NotificationTableDeleteAction extends AbstractAction<Table> {
    ...
    public async run(event: any): Promise<void> {
        if (this.canRun()) {
            ...
            const content = new ComponentContent(
                'confirm-overlay',
                new ConfirmOverlayContent(question, this.deleteNotifications.bind(this))
            );
            ...
            OverlayService.getInstance().openOverlay(
                ...
                null,
                content,
                'Translatable#Remove Notifications',
                null,
                false
            );
        }
    ...
    public async deleteNotifications(): Promise<void> {
        ...
        if (selectedRows && !!selectedRows.length) {
            EventService.getInstance().publish(ApplicationEvent.APP_LOADING, {
                loading: true, hint: 'Translatable#Remove Notifications'
            });
            const failIds = await KIXObjectService.deleteObject(
                KIXObjectType.NOTIFICATION, selectedRows.map((sR) => sR.getRowObject().getObject().ObjectId)
            );
            ...
            if (!failIds || failIds.length === 0) {
                const content = new ComponentContent(
                    'toast',
                    new ToastContent('kix-icon-check', 'Translatable#Notifications successfully removed.')
                );
