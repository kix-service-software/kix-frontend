...
export class MailAccountTableDeleteAction extends AbstractAction<Table> {
    ...
    public async run(event: any): Promise<void> {
        if (this.canRun()) {
            ...
            const content = new ComponentContent(
                'confirm-overlay',
                new ConfirmOverlayContent(question, this.deleteMailAccounts.bind(this))
            );
            ...
            OverlayService.getInstance().openOverlay(
                ...
                null,
                content,
                'Translatable#Remove Mail Accounts',
                null,
                false
            );
        }
    ...
    public async deleteMailAccounts(): Promise<void> {
        const selectedRows = this.data.getSelectedRows();
        if (selectedRows && !!selectedRows.length) {
            EventService.getInstance().publish(ApplicationEvent.APP_LOADING, {
                loading: true, hint: 'Translatable#Remove Mail Accounts'
            });
            const failIds = await KIXObjectService.deleteObject(
                KIXObjectType.MAIL_ACCOUNT, selectedRows.map((sR) => sR.getRowObject().getObject().ObjectId)
            );
            ...
            if (!failIds || failIds.length === 0) {
                const content = new ComponentContent(
                    'toast',
                    new ToastContent('kix-icon-check', 'Translatable#Mail Accounts successfully removed.')
                );
