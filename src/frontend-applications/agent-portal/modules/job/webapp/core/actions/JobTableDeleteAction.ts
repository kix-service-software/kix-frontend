...
export class JobTableDeleteAction extends AbstractAction<Table> {
    ...
    public async run(event: any): Promise<void> {
        if (this.canRun()) {
            ...
            const content = new ComponentContent(
                'confirm-overlay',
                new ConfirmOverlayContent(question, this.deleteJobs.bind(this))
            );
    ...
    public async deleteJobs(): Promise<void> {
        const selectedRows = this.data.getSelectedRows();
        if (selectedRows && !!selectedRows.length) {
            EventService.getInstance().publish(ApplicationEvent.APP_LOADING, {
                loading: true, hint: 'Translatable#Remove Jobs'
            });
            const failIds = await KIXObjectService.deleteObject(
                KIXObjectType.JOB, selectedRows.map((sR) => sR.getRowObject().getObject().ObjectId)
            );
            ...
            if (!failIds || failIds.length === 0) {
                const content = new ComponentContent(
                    'toast',
                    new ToastContent('kix-icon-check', 'Translatable#Jobs successfully removed.')
                );
