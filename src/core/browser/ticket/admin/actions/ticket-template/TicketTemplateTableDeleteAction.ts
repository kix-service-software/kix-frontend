...
export class TicketTemplateTableDeleteAction extends AbstractAction<ITable> {
    ...
    //             'confirm-overlay',
    //             new ConfirmOverlayContent(question, this.deleteTemplates.bind(this))
    //         );
    ...
    public async deleteTemplates(): Promise<void> {
        ...
        if (selectedRows && !!selectedRows.length) {
            EventService.getInstance().publish(ApplicationEvent.APP_LOADING, {
                loading: true, hint: 'Translatable#Remove Templates'
            });
            const failIds = await KIXObjectService.deleteObject(
                KIXObjectType.TICKET_TEMPLATE, selectedRows.map((sR) => sR.getRowObject().getObject().ObjectId)
            );
            ...
            if (!failIds || !!!failIds.length) {
                const content = new ComponentContent(
                    'toast',
                    new ToastContent('kix-icon-check', 'Translatable#Templates successfully removed.')
                );
